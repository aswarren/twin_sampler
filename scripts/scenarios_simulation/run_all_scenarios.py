...
def main():
    ...
    figA.tight_layout()
    outA = outdir / "A_table3_targets_1x3.png"
    ...
    for ax, algo in zip(axesB, algo_list):
        ...
        ax.grid(True, linestyle="--", alpha=0.6); ax.legend(ncol=4, fontsize=8); ax.set_xlim(left=0.9)
    figB.tight_layout()
    outB = outdir / "B_vs_cumulative_infections_1x3.png"
    plt.savefig(outB, dpi=150); print(f"Saved: {outB}")
    plt.close(figB)

    # =================== FIGURE C: rolling infections (1×3) ===================
    figC, axesC = plt.subplots(1, 3, figsize=(21, 6), sharey=True)
